...
│   ├── branch-reviewer/     # Review the current branch against a target branch
│   └── convert-frontend-to-skill/  # Analyze a frontend codebase to generate a backend API skill
...
| `branch-reviewer` | Review the current branch against a target branch with an executive summary, technical analysis, security profile, and deployment risk assessment |

### Skill Generation

| Skill | Description |
|-------|-------------|
| `convert-frontend-to-skill` | Analyze a frontend codebase to discover backend API resources and generate a new skill that can interact with those APIs directly |
...
3. Create any required storage subdirectories.
